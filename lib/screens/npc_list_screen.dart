          ...
          children: [
            Row(
              children: [
                ...
                TerminalText(npc.alive ? 'Geracao ${npc.generation}' : 'MORTO',
            ...
            Wrap(
              ...
              runSpacing: 2,
              children: [
                TerminalText('Origem: ${npc.origin.label}', fontSize: 9, color: AppTheme.cyan),
                TerminalText('Funcao: ${npc.profession.label}', fontSize: 9, color: AppTheme.textSecondary),
                TerminalText('${npc.age} anos', fontSize: 9, color: AppTheme.textDim),
              ],
            ),
            ...
            Wrap(
              ...
              children: [
                ...
                if (npc.fame < 0)
                  TerminalText('Infame: ${npc.fame.toStringAsFixed(0)}', fontSize: 9, color: AppTheme.red),
                TerminalText('Leal: ${npc.loyalty.toStringAsFixed(0)}%', fontSize: 9,
                    color: npc.loyalty > 60 ? AppTheme.green : npc.loyalty > 30 ? AppTheme.yellow : AppTheme.red),
              ],
            ),
            if (npc.talentDiscovered && npc.hiddenTalent != HiddenTalent.none)
              Padding(
                padding: const EdgeInsets.only(top: 3),
                child: TerminalText('Talento: ${npc.hiddenTalent.label}', fontSize: 9, color: AppTheme.purple),
              ),
            if (npc.isSuspicious || npc.calculatedBetrayalRisk > 30)
              Padding(
                padding: const EdgeInsets.only(top: 3),
                child: Row(
                  children: [
                    TerminalText(
                      'ALERTA: Risco de traicao ${npc.calculatedBetrayalRisk.toStringAsFixed(0)}%',
                      fontSize: 9, color: AppTheme.red,
                    ),
                    if (npc.origin.isDarkOrigin)
                      TerminalText(' [${npc.origin.label}]', fontSize: 9, color: AppTheme.red),
                  ],
                ),
              ),
            if (npc.groupId != null)
              Padding(
                ...
                child: Builder(builder: (_) {
                  final group = gp.groups.where((g) => g.id == npc.groupId).firstOrNull;
                  return TerminalText('Grupo: ${group?.name ?? "Desconhecido"}', fontSize: 9, color: AppTheme.blue);
                }),
              ),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(children: [
                ...
                TerminalText(npc.alive ? 'VIVO' : 'MORTO', fontSize: 10, color: npc.alive ? AppTheme.green : AppTheme.red),
              ]),
              const SizedBox(height: 4),
              TerminalText(
              ...
              StatBar(label: 'Forca', value: npc.attributes.strength, maxValue: 15),
              StatBar(label: 'Agil.', value: npc.attributes.agility, maxValue: 15),
              StatBar(label: 'Intel.', value: npc.attributes.intelligence, maxValue: 15),
              StatBar(label: 'Resist.', value: npc.attributes.endurance, maxValue: 15),
              StatBar(label: 'Caris.', value: npc.attributes.charisma, maxValue: 15),
              ...
              TerminalText('Funcao atual: ${npc.profession.label}', fontSize: 10, color: AppTheme.textPrimary),
              const CyanDivider(label: 'ESTATISTICAS'),
              TerminalText('Andares superados: ${npc.floorsCleared}', fontSize: 9, color: AppTheme.textSecondary),
              TerminalText('Fama acumulada: ${npc.fame.toStringAsFixed(0)}', fontSize: 9, color: AppTheme.yellow),
              TerminalText('Estado mental: ${npc.calculatedMentalCondition.label}', fontSize: 9, color: AppTheme.textSecondary),
              const CyanDivider(label: 'SOCIAL'),
              StatBar(label: 'Leal.', value: npc.loyalty, maxValue: 100,
                  color: npc.loyalty > 60 ? AppTheme.green : npc.loyalty > 30 ? AppTheme.yellow : AppTheme.red),
              TerminalText('Reputacao: ${npc.fameLabel} (${npc.fame.toStringAsFixed(0)})', fontSize: 9,
                  color: npc.fame >= 0 ? AppTheme.yellow : AppTheme.red),
              if (npc.calculatedBetrayalRisk > 10)
                TerminalText('Risco de traicao: ${npc.calculatedBetrayalRisk.toStringAsFixed(0)}%', fontSize: 9,
                    color: npc.calculatedBetrayalRisk > 50 ? AppTheme.red : AppTheme.orange),
              if (npc.groupId != null)
                Builder(builder: (_) {
                  final group = gp.groups.where((g) => g.id == npc.groupId).firstOrNull;
                  return TerminalText('Grupo: ${group?.name ?? "Sem grupo"}', fontSize: 9, color: AppTheme.blue);
                }),
              if (npc.trainingSuggestionsReceived > 0)
                TerminalText('Sugestoes: ${npc.trainingSuggestionsAccepted}/${npc.trainingSuggestionsReceived} aceitas',
                    fontSize: 9, color: AppTheme.textDim),
              if (npc.origin.isDarkOrigin)
                TerminalText('ORIGEM OBSCURA: ${npc.origin.label}', fontSize: 9, color: AppTheme.red),
              if (npc.partnerId != null)
                Builder(builder: (_) {
                  final partner = gp.allNpcs.where((n) => n.id == npc.partnerId).firstOrNull;
                  return TerminalText('Parceiro(a): ${partner?.name ?? "Desconhecido"}', fontSize: 9, color: AppTheme.pink);
                }),
